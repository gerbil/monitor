...
class Jenkins_Job
{
     ...
     * @return string
     */
    public function getHealthReportDesc()
    {
        return $this->job->healthReport[0]->description;
    }
    ...
    /**
     * @param int $buildId
     * @return string $logs|null
     */
    public function getLogs($buildId)
    {
        $url = sprintf('%s%d/logText/progressiveText?start=0', $this->job->url, $buildId);
        $curl = curl_init($url);
